...
{
    BlockLayoutTransformer(Net* net_) : net(net_), netimpl(net_->getImpl()) {}
    ...
    void transformGraph(Graph& g)
    {
        ...
        for (const Node& node: curr_prog) {
            ...
            TensorLayout layout1 = op_blockiness > 0 ? DATA_LAYOUT_BLOCK : op_blockiness < 0 ? defaultLayout : layout0;
