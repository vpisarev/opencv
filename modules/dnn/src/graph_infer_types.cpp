// This file is part of OpenCV project.
// It is subject to the license terms in the LICENSE file found in the top-level directory
// of this distribution and at http://opencv.org/license.html.

#include "precomp.hpp"
#include "net_impl.hpp"

namespace cv { namespace dnn {
CV__DNN_INLINE_NS_BEGIN

#if 0
using std::vector;
using std::string;

typedef std::pair<int, int> int_pair;
typedef std::pair<int, Arg> int_arg_pair;

struct InferTypes
{
    InferTypes(Net* net_) : net(net_), netimpl(net_->getImpl()) {}

    void infer()
    {
        // reset types first
        size_t i, nargs = netimpl->args.size();
        for (i = 0; i < nargs; i++) {
            ArgData& adata = netimpl->args[i];
            if (adata.kind != DNN_ARG_INPUT && adata.kind != DNN_ARG_CONST) {
                adata.type = -1;
            }
        }
        inferGraph(netimpl->mainGraph);
    }

    bool inferGraph(const Ptr<Graph>& graph)
    {
        const std::vector<Ptr<Layer> >& prog = graph->prog();
        size_t i, j, nargs = netimpl->args.size(), nops = prog.size();
        std::vector<int> inptypes, outtypes;

        for (i = 0; i < nops; i++) {
            const Node& node = prog[i];
            std::vector<Graph>& subgraphs = const_cast<std::vector<Graph>&>(node->subgraphs());
            for (Graph& g: subgraphs) {
                inferGraph(g);
            }
            const std::vector<Arg>& inputs = node->inputs();
            const std::vector<Arg>& outputs = node->outputs();
            size_t ninputs = inputs.size();
            size_t noutputs = outputs.size();
            const Op& op = node->op();
            inptypes.resize(ninputs);
            outtypes.resize(noutputs);

            for (j = 0; j < ninputs; j++) {
                const ArgData& adata = netimpl->args[inputs[j].idx];
                inptypes[j] = adata.type;
            }
            op->inferTypes(*net, graph, inputs, inptypes, outputs, outtypes);
            for (j = 0; j < noutputs; j++) {
                ArgData& adata = netimpl->args[outputs[j].idx];
                adata.type = outtypes[j];
            }
        }
    }

    Net* net;
    Net::Impl* netimpl;
};

void Net::Impl::inferTypes()
{
    InferTypes typeInferencer(net);
    typeInferencer.infer();
}
#endif

CV__DNN_INLINE_NS_END
}}
